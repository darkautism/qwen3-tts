...
| **Predictor** | CodePredictor（預設 Candle GGUF）+ ONNX 回退 + 回饋嵌入 | CPU | 9091 |
| **Vocoder** | Vocoder (ONNX FP32 CPU) | CPU | 9092 |

將工作分散在低成本 SBC 或任何 Linux 機器上。Token 生成：~5.5 tok/s（使用精簡版 code-predictor GGUF）。
Predictor 後端預設走 GGUF（Candle/GGML 路徑）；ONNX 僅作回退/診斷用途。
...
`ggml-predictor` / `ggml-backend` 需要原生 C/C++ 靜態庫。  
目前 `build.rs` 會從 `third_party/qwen3-tts.cpp`（submodule）自動初始化並自動編譯，所以 `cargo install --git` 可直接建置 GGML 模式。

需求工具：`git`、`cmake`、C++ 編譯器（`c++`）與 `ar`（binutils）。

1) 推薦：一條命令安裝（自動 submodule + 自動編譯原生庫）：

```bash
# 只加速 predictor（talker 維持 Candle）
cargo install --git "https://github.com/darkautism/qwen3-tts" \
  --locked qwen3-tts-rs \
  --features ggml-predictor

# talker + predictor 都走 GGML
cargo install --git "https://github.com/darkautism/qwen3-tts" \
  --locked qwen3-tts-rs \
  --features ggml-backend
```

`build.rs` 會自動生成/檢查以下檔案：
...
2) 可選覆蓋：改用你自己的 `qwen3-tts.cpp` source 路徑：
...
# build.rs 會使用 GGML_LIB_DIR 並補齊缺失的原生庫
cargo build --release --features ggml-predictor
```

3)（僅 `ggml-backend` 需要）從 `llama.cpp` source 編譯 `llama_wrapper.so`：
...
# 日誌應看到："Using GGML code predictor (optimized C backend)"
```
...
> 預設 chunk 模式為 `none`（不分段）。只有在你想做標點分段時才使用 `--chunk 2` 或 `--chunk 4`。
...
| `qwen3-tts speak "文字" --chunk none` | 不分段（預設模式） |
...
| `chunk_mode` | string | `"none"` | 文字分段模式（`none`、`2`、`4`） |
